...
function displayNotification(user, json) {
    ...
    var message = `NEW OWNER! <a href="/api/owners/${data.firstName}">${data.firstName}</a>, ${data.middleName}, ${data.lastName}, ${data.serial + "&" + data.number}`;
...
function displayNotificationVehicle(user, json) {
    console.log(json);
    var $target = $('div#signalr-notifications');
    var data = JSON.parse(json);
...
function connectToSignalR() {
    ...
    conn.on("DisplayNotificationVehicle",displayNotificationVehicle);
